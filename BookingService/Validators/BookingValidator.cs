...
public class BookingValidator : IBookingValidator
{
    ...
    {
        ValidatePassengerCount(request.Passengers);
        ValidateLeadPassenger(request.Passengers);
        ValidatePassengerDetails(request.Passengers);
    ...
    private void ValidatePassengerCount(List<CreatePassengerRequest> passengers)
    {
        if (passengers == null)
    ...
    private void ValidateLeadPassenger(List<CreatePassengerRequest> passengers)
    {
        if (passengers.Count(p => p.IsLeadPassenger) != 1)
            throw new ArgumentException("Exactly one lead passenger must be designated");
    }

    private void ValidatePassengerDetails(List<CreatePassengerRequest> passengers)
    {
    ...
    private void ValidateFlightInfo(CreateBookingRequest request)
    {
    ...
    private void ValidateBookingDetails(CreateBookingRequest request)
    {
        ...
        if (!Regex.IsMatch(request.ContactEmail, @"^[^@\s]+@[^@\s]+\.[^@\s]+$", RegexOptions.None, TimeSpan.FromMilliseconds(100)))
