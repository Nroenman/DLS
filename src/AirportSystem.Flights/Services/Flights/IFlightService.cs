...
public interface IFlightService
{
    ...
    Task<IReadOnlyList<Flight>> GetBookedFlightsByUserAsync(Guid userId);
    Task<IReadOnlyList<Flight>> GetFollowedFlightsByUserAsync(Guid userId);

    Task<Booking> BookFlightAsync(Guid userId, Guid flightId, string? seatNumber = null);
    Task<bool> UnbookFlightAsync(Guid userId, Guid flightId);
