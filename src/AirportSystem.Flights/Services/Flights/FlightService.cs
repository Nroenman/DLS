...
public class FlightService : IFlightService
{
    ...
    public async Task<IReadOnlyList<Flight>> GetBookedFlightsByUserAsync(Guid userId)
    {
        return await _db.Bookings
            .Where(b => b.UserId == userId)
            .Include(b => b.Flight).ThenInclude(f => f.Gate)
            .Select(b => b.Flight)
            .OrderBy(f => f.ScheduledDeparture)
            .ToListAsync();
    }
    ...
    public async Task<Booking> BookFlightAsync(Guid userId, Guid flightId, string? seatNumber = null)
    {
        ...
        if (await _db.Bookings.AnyAsync(b => b.UserId == userId && b.FlightId == flightId))
            throw new InvalidOperationException("You have already booked this flight.");
    ...
    public async Task<bool> UnbookFlightAsync(Guid userId, Guid flightId)
    {
        var booking = await _db.Bookings
            .FirstOrDefaultAsync(b => b.UserId == userId && b.FlightId == flightId);
