...
public class Mutation
{
    ...
    [Authorize]
    [GraphQLDescription("Book a flight for the currently authenticated user.")]
    public async Task<BookingPayload> BookFlight(
        ...
        [Service] IHttpContextAccessor httpContextAccessor)
    {
        ...
        var booking = await flightService.BookFlightAsync(user.Id, input.FlightId, input.SeatNumber);
        return new BookingPayload(booking);
    }

    [Authorize]
    [GraphQLDescription("Cancel a booking for the currently authenticated user.")]
    public async Task<UnbookPayload> UnbookFlight(
        UnbookFlightInput input,
        [Service] IFlightService flightService,
        [Service] IHttpContextAccessor httpContextAccessor)
    {
        var userId = httpContextAccessor.HttpContext!.User.GetUserId();
        var success = await flightService.UnbookFlightAsync(userId, input.FlightId);
        return new UnbookPayload(success);
    }
